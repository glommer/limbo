    ...
    walk_expr_mut(
        ...
        &mut |expr: &mut ast::Expr| -> Result<WalkControl> {
            match expr {
                ...
                Expr::Name(name) => {
                    // Handle Expr::Name the same way as Expr::Id
                    // This is needed for PostgreSQL translator which produces Name instead of Id
                    let Some(referenced_tables) = &mut referenced_tables else {
                        if binding_behavior == BindingBehavior::AllowUnboundIdentifiers {
                            return Ok(WalkControl::Continue);
                        }
                    ...
                    for joined_table in referenced_tables.joined_tables().iter() {
                        ...
                        if col_idx.is_some() {
                            if match_result.is_some() {
                                ...
                                if let Some(join_info) = &joined_table.join_info {
                                    if join_info.using.iter().any(|using_col| {
                                        using_col.as_str().eq_ignore_ascii_case(&normalized_name)
                                    }) {
                                        ok = true;
                                    }
                                }
                                if !ok {
                                    crate::bail_parse_error!("Column {} is ambiguous", name.as_str());
                                }
                            } else {
                                let col =
                                    joined_table.table.columns().get(col_idx.unwrap()).unwrap();
                    ...
                    // For Expr::Name, we don't have quote information, so we can't fall back to string literals
                    // Just fail with column not found error
                    crate::bail_parse_error!("no such column: {}", name.as_str())
                }
