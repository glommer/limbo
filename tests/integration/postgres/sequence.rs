...
fn pg_execute(conn: &Arc<Connection>, sql: &str) {
...
fn pg_assert_no_rows(conn: &Arc<Connection>, sql: &str) {
    let mut rows = conn.query(sql).unwrap().unwrap();
    match rows.step().unwrap() {
        StepResult::Done => {}
...
fn pg_execute_err(conn: &Arc<Connection>, sql: &str) -> String {
...
fn pg_query_err(conn: &Arc<Connection>, sql: &str) -> String {
